...
describe 'Create Message', type: :request do
  subject { post api_v1_messages_path(conversation_id: conversation.id), params:, headers:, as: :json }

  let(:user1) { create(:user) }
  let(:user2) { create(:user) }
  let(:user3) { create(:user) }
  let(:conversation) { create(:conversation, user1:, user2:) }
